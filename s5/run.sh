...
steps/train_mmi_sgmm2.sh --cmd "$decode_cmd" \
 --transform-dir exp/tri3_ali --boost 0.1 \
 data/train data/lang exp/sgmm2_4_ali exp/sgmm2_4_denlats \
 exp/sgmm2_4_mmi_b0.1
...
# Here I use Neural Network in {src/nnet src/nnetbin} scripts in ./steps/nnet/* 
. ./run_dnn.sh
...
#nnet_dir=dnn4b_pretrain-dbn_dnn

nnet_dir=dnn4b_pretrain-dbn_dnn_smbr_i1lats

for iter in 1 2 3 4; do
  local/score_combine.sh --cmd "$decode_cmd" \
   data/dev data/lang_test_bg exp/$nnet_dir/decode_dev_iter1 \
   exp/sgmm2_4_mmi_b0.1/decode_dev_it$iter exp/combine_2_smbr/decode_dev_it$iter

  local/score_combine.sh --cmd "$decode_cmd" \
   data/test data/lang_test_bg exp/$nnet_dir/decode_test_iter1 \
   exp/sgmm2_4_mmi_b0.1/decode_test_it$iter exp/combine_2_smbr/decode_test_it$iter
done
